...
class GameEngine:
    def topic_reroll_needed(self, room: dict) -> int:
        return len(room["players"]) // 2 + 1

    def apply_new_topic(self, room: dict, status_text: str) -> None:
        if not self.topics:
        ...
        room["topic_change_event_id"] += 1
        room["status_text"] = status_text

    def vote_reroll_topic(self, room_code: str, requester: str) -> str:
        ...
        if room["phase"] != "preparation":
            raise ValueError("Голосовать за смену темы можно только на этапе подготовки.")

        if requester in room["topic_reroll_voters"]:
            raise ValueError("Ты уже голосовал за смену темы.")

        room["topic_reroll_voters"].add(requester)

        if len(room["topic_reroll_voters"]) >= self.topic_reroll_needed(room):
            self.apply_new_topic(room, "Тема изменена голосованием.")
            return "rerolled"

        room["status_text"] = "Идёт голосование за смену темы."
        return "waiting"
    def reroll_topic(self, room_code: str, requester: str) -> None:
        ...
        self.apply_new_topic(room, "Хост сменил тему.")
    ...
    def create_room(self, name: str) -> str:
        ...
        self.rooms[code] = {
            ...
            "topic": None,
            "spy_name": None,
            "topic_reroll_voters": set(),
            "topic_change_event_id": 0,
            "spy_inserted": False,
    ...
    def player_view(self, room: dict, player_name: str) -> dict:
        ...
        return {
            "topic_reroll_votes_count": len(room["topic_reroll_voters"]),
            "topic_reroll_votes_needed": self.topic_reroll_needed(room),
            "can_vote_topic_reroll": (
                    room["phase"] == "preparation"
                    and player_name not in room["topic_reroll_voters"]
            ),
            "topic_change_event_id": room["topic_change_event_id"],
            "vote_progress_count": max(len(room["current_vote_ballots"]) - 1, 0),
            "vote_progress_total": sum(
                1
                for p in room["players"]
                if p["connected"] and p["name"] != room["voting_target"]
            ) if room["phase"] == "voting" and room["voting_target"] else 0,
            "room_code": room["code"],
            "phase": room["phase"],
